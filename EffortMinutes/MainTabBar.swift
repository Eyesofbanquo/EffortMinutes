//
//  MainTabBar.swift
//  EffortMinutes
//
//  Created by Markim Shaw on 1/18/22.
//

import Foundation
import UIKit

final class MainTabBar: UIView, MainTabBarItemDelegate {
  
  var stackView: UIStackView
  var items: [MainTabBarItem] = []
  weak var delegate: MainTabBarItemDelegate?
  
  init() {
    stackView = UIStackView()
    stackView.translatesAutoresizingMaskIntoConstraints = false
    
    stackView.axis = .horizontal
    stackView.spacing = 4.0
    stackView.alignment = .center
    stackView.distribution = .equalSpacing
    
    super.init(frame: .zero)
    
    backgroundColor = .init(hexString: "#CCDBDC")
    
    addSubview(stackView)
    stackView.snp.makeConstraints { make in
      make.leading.top.equalTo(self).offset(8.0)
      make.bottom.trailing.equalTo(self).offset(-8.0)
    }
  }
  
  required init?(coder: NSCoder) {
    fatalError()
  }
  
  func add(item: MainTabBarItem) {
    stackView.insertArrangedSubview(item, at: item.index)
    item.delegate = self
  }
  
  func mainTabBarItem(_ item: MainTabBarItem, didSelectIndex index: Int) {
    /* Do something with THIS tab bar's delegate */
    for case let tab as MainTabBarItem in stackView.arrangedSubviews where tab.index != index {
      tab.deselect()
    }
    self.delegate?.mainTabBarItem(item, didSelectIndex: index)
    
    
//    for item in items where item.index != index {
//      guard item.index != index else { continue }
//      item.deselect()
//    }
  }
}
